...
async function getFunctionSuggestionForIssue(item, functionOptions) {
  ...
  const aiSpinner = ora('Getting function suggestion from ChatGPT...').start();
...
Please respond ONLY with the exact name of one of the valid functions listed above, no explanation or additional text.`;
    ...
    aiSpinner.succeed(`ChatGPT suggestion: ${functionSuggestion}`);
    ...
    aiSpinner.fail(`Error getting function suggestion: ${error.message}`);
    return '';
  }
}
...
export async function fixFunctionField(options) {
  ...
  try {
    ...
    const mainSpinner = ora('Starting function field fix process...').start();
    ...
    if (!functionField) {
      mainSpinner.fail('No function field found in this project.');
      return 0;
    }
    
    mainSpinner.succeed(`Found function field with ${functionField.options.length} options.`);
    ...
    itemsSpinner.text = 'Filtering items missing function field...';
    ...
    itemsSpinner.succeed(`Found ${itemsWithoutFunction.length} items without function field set.`);
    let updatedCount = 0;
    ...
    for (const item of itemsWithoutFunction) {
      ...
      const functionOption = functionField.options.find(option => {
        ...
        return option.name === functionName;
      });
      ...
      const updateSpinner = ora(`Updating function field for issue #${item.content.number}...`).start();
      ...
      try {
        ...
        updatedCount++;
        updateSpinner.succeed(`Updated function for issue ${item.content.number} to "${functionOption.name}"`);
      } catch (error) {
        updateSpinner.fail(`Error updating function for issue ${item.content.number}: ${error.message}`);
      }
    }
    ...
    ora().fail('Error fixing function fields');
