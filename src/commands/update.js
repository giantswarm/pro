...
export async function updateCommand(options) {
  try {
    if (!options.id || !options.title) {
      console.error(chalk.red('Project ID and title are required for updating the Project v2 board.'));
      return;
    }
    ...
    console.log(chalk.green(`Updated Project v2 board: [ID: ${project.id}] ${project.title}`));
  } catch (error) {
    console.error(chalk.red('Error updating Project v2 board:'), chalk.red(error.message));
  }
}
